...
BENCH_FILE_NAME = "*seguimiento al - PHOENIX*.xlsx"
...
def get_input_excel_path() -> Path:
    env_path = os.getenv("BENCH_STH_EXCEL_PATH", "").strip()
    if env_path:
        return Path(env_path)
    return BENCH_FOLDER / BENCH_FILE_NAME
    ...
    with connect() as cn:
        ...
        for original in excel_cols:
            if col_is_mm_monto(original):
                cur.execute(
                    f"ALTER TABLE {TABLE} ALTER COLUMN {sql_ident(f'fld_{original}')} DECIMAL(38,2) NULL;"
                )
