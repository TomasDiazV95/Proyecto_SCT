...
CSV_FOLDER = Path(os.getenv("ASIGNACION_ITAU_VENCIDA_FOLDER", str(DEFAULT_FOLDER)))
CSV_PATTERN = os.getenv("ASIGNACION_ITAU_VENCIDA_PATTERN", "*.csv")

TABLE = "dbo.asignacion_itau_vencida"
BATCH_SIZE = 10000

SOURCE_COLUMN_SPECS = [
    ("Tipo_Asignacion", "text20"),
    ("Segmento", "text100"),
    ("Fecha", "date"),
    ("Apellido_Cliente", "text150"),
    ("Nombre_Cliente", "text40"),
    ("Numero_Cliente", "text20"),
    ("Numero_Cuenta", "text24"),
    ("Producto", "text40"),
    ("Subproducto", "text40"),
    ("Codigo_Subproducto", "text20"),
    ("Fecha_Fin_Asignacion", "date"),
    ("Escenario_en_Asg", "text50"),
    ("Estrategia_en_Asg", "text100"),
    ("Estado_en_Asg", "text40"),
    ("Escenario_actual", "text50"),
    ("Estrategia_actual", "text40"),
    ("Estado_actual", "text50"),
    ("Dias_Mora", "integer"),
    ("Deuda_Vencida", "integer"),
    ("Deuda_A_Vencer", "integer"),
...
def read_csv_file(path: Path) -> pd.DataFrame:
    ...
    for encoding in ("utf-8-sig", "utf-16", "utf-16-le", "cp1252", "latin1"):
        try:
            ...
            rut_dv = df["Numero_Cliente"].apply(split_rut_dv)
            df.insert(df.columns.get_loc("Numero_Cliente") + 1, "Rut", rut_dv.apply(lambda item: item[0]))
            df.insert(df.columns.get_loc("Rut") + 1, "Dv", rut_dv.apply(lambda item: item[1]))

            invalid_rut = df["Numero_Cliente"].apply(clean_cell).notna() & (
                df["Rut"].isna() | df["Dv"].isna()
            )
            if invalid_rut.any():
                print(f"Advertencia: {int(invalid_rut.sum())} filas tienen Numero_Cliente invalido")
        ...
        cur.execute(
            ...
            BEGIN
                CREATE TABLE {TABLE} (
                    ...
                    fecha_carga DATE NOT NULL CONSTRAINT DF_asignacion_itau_vencida_fecha_carga DEFAULT (CONVERT(date, GETDATE())),
                    ts_carga DATETIME2(0) NOT NULL CONSTRAINT DF_asignacion_itau_vencida_ts_carga DEFAULT (SYSDATETIME()),
                    source_file NVARCHAR(260) NULL,
                    {column_definitions}
                );
                CREATE INDEX IX_asignacion_itau_vencida_fecha_carga ON {TABLE}(fecha_carga);
                CREATE INDEX IX_asignacion_itau_vencida_source_file ON {TABLE}(source_file);
                CREATE INDEX IX_asignacion_itau_vencida_rut ON {TABLE}(Rut);
            END
            """
        )
        cn.commit()
...
def main() -> None:
    ...
    print(f"Archivo asignacion Itau vencida: {csv_path}")
