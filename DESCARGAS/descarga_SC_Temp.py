...
CARPETA_BASE = Path(r"C:\Users\PC del Marrón\Desktop\Paso")
...
def buscar_archivo_bench(carpeta: Path) -> Path:
    ...
    for archivo in archivos:
        ...
        if "BENCH MORA TEMPRANA - PHOENIX (TELEFONIA)" in nombre:
            return archivo

    raise FileNotFoundError(
        f"No encontré un archivo con 'BENCH MORA TEMPRANA - PHOENIX (TELEFONIA)' en {carpeta}"
    )


def ejecutar_etl(archivo_bench: Path) -> None:
    env = os.environ.copy()
    env["BENCH_TEMP_EXCEL_PATH"] = str(archivo_bench)

    etl_path = BASE_DIR.parent / "ETL" / "etl_bench_temp_stc.py"
    print(f"Ejecutando ETL: {etl_path}")

    subprocess.run(
        [sys.executable, str(etl_path)],
        check=True,
        env=env,
        cwd=str(etl_path.parent),
    )
...
def run(playwright: Playwright) -> None:
    ...
    frame.get_by_role("link", name="📁 BENCH MORA TEMPRANA").wait_for()
    frame.get_by_role("link", name="📁 BENCH MORA TEMPRANA").click()

    frame.get_by_role("button", name="FECHA DE MODIFICACIÓN ↓↑").click()
    frame.locator("#rptEntriesWFM_chkSelectWFM_1").check()
